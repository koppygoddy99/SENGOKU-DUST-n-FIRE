...
| 0.10 | 2026-09-04 | **Cline rules + skills contract** (docs-only, no engine/gameplay change): created the `.clinerules/` contract — 00-project-rules (permanent principles: roadmap/push gate, gameplay safety, smallest safe change, verification), 10-architecture (evidence-based map of client shell, engine/state, play flow, AI GM boundary, persistence, UI projections, tests), 20-gameplay-safety (BLOCKED — GAMEPLAY DECISION REQUIRED protocol, gameplay vs implementation decisions), 30-workflow (standard task flow), 40-performance (search-first investigation) — plus five skills under `.clinerules/skills/`: repo-investigation, gameplay-implementation, testing-verification, roadmap-audit, and ui-implementation. All rule/skill content audited against live repository code; no system status changed. |
