...
function OutcomeCard({ record, game, language, notice, onContinue, onMap, onChronicle }: { record: OutcomeRecord; game: GameState; language: Language; notice: string; onContinue: () => void; onMap: () => void; onChronicle: () => void }) {
  ...
  return <section className="play-outcome-card"><header><div><p className="play-scene__eyebrow">{copy(language, "OUTCOME · SAVED TO CAMPAIGN", "ผลลัพธ์ · บันทึกลงแคมเปญแล้ว")}</p><h2>{outcomeLabel(record.outcome)}</h2></div><span>{copy(language, "Leaf", "หน้า")} {game.progression?.leaf ?? record.tick}</span></header>{notice && <p className="play-scene__notice play-outcome-card__notice">{notice}</p>}<div className="play-outcome-card__layers"><article><small>{copy(language, "1 · ROLL RESULT", "1 · ผลทอย")}</small><strong>{record.total} / DN {record.difficulty}</strong><p>{record.summary}</p></article><article><small>{copy(language, "2 · SKILL PRACTICE", "2 · วิชาที่ฝึก")}</small><strong>{practice ? `${practice.masteryLabel} ${practice.gained ? `+${practice.gained} XP` : copy(language, "held", "ยังไม่ขยับ")}` : copy(language, "No new practice", "ไม่มีวิชาใหม่")}</strong><p>{practice ? `${copy(language, "Step", "ขั้น")} ${practice.rankAfter} · ${practice.masteryMark ?? `${practice.xp}/${practice.xpNeeded} XP`} · ${practice.note ?? ""}` : ""}</p></article><article><small>{copy(language, "3 · TIME & THREAD", "3 · เวลาและเส้นเรื่อง")}</small><strong>{timeMark?.message ?? copy(language, "The scene holds", "ฉากยังคงอยู่")}</strong><p>{missionUpdate ? `${missionUpdate.current}/${missionUpdate.required} · ${missionUpdate.state}${missionUpdate.reward ? ` · ${missionUpdate.reward}` : ""}` : copy(language, "No mission change", "ภารกิจยังไม่เปลี่ยน")}</p></article><article className="play-outcome-card__prose"><small>{copy(language, "4 · STORY RESULT", "4 · ผลเชิงเรื่องเล่า")}</small><strong>{game.currentScene.title}</strong><TypewriterProse text={narrative} /><p>{worldTrace?.detail ?? record.consequence}</p></article></div><div className="play-outcome-card__actions"><Button className="df-button df-button--primary" onClick={onContinue}>{copy(language, "CONTINUE SCENE", "เล่นฉากต่อ")} <ArrowRight size={17} /></Button><Button className="df-button df-button--ghost" onClick={onMap}><Map size={16} /> {copy(language, "RETURN TO MAP", "กลับแผนที่")}</Button><button onClick={onChronicle}><FileText size={15} /> {copy(language, "OPEN CHRONICLE", "เปิดบันทึกเรื่อง")}</button></div></section>;
}
...
export function PlayScene({ game, language, onOpen, onUpdate, isAuthenticated, uiPreviewMode, onLogin, onAccountCreditChange }: { game: GameState; language: Language; onOpen: (page: PlayDestination) => void; onUpdate: (next: GameState, message: string) => void; isAuthenticated: boolean; uiPreviewMode: boolean; onLogin: () => void; onAccountCreditChange: () => unknown }) {
  ...
  const resetIntent = () => { setDetails(null); setDiceResult(null); setRolling(false); setNotice(""); };
  const commitIntent = () => {
  ...
  const roll = () => { if (!details) return; const record = resolveRoll(details, game, false); setDiceResult(record); const reduced = window.matchMedia?.("(prefers-reduced-motion: reduce)").matches ?? true; if (reduced) { setDisplayDice(record.dice); setNotice(copy(language, "The dice are visible. Momentum may be spent now, before this result is recorded.", "เห็นผลลูกเต๋าแล้ว จะใช้แรงฮึดได้ตอนนี้ก่อนบันทึกผล")); return; } setRolling(true); const interval = window.setInterval(() => setDisplayDice([Math.floor(Math.random() * 12) + 1, Math.floor(Math.random() * 12) + 1]), 90); window.setTimeout(() => { window.clearInterval(interval); setDisplayDice(record.dice); setRolling(false); setNotice(copy(language, "The dice are visible. Momentum may be spent now, before this result is recorded.", "เห็นผลลูกเต๋าแล้ว จะใช้แรงฮึดได้ตอนนี้ก่อนบันทึกผล")); }, ROLL_ANIMATION_MS); };
  ...
  return <div className="page play-scene-page">
    ...
    {outcome ? <OutcomeCard record={outcome} game={game} language={language} notice={notice} onContinue={continueScene} onMap={() => onOpen("home")} onChronicle={() => onOpen("log")} /> : <section className="play-scene__paper">
      ...
      <section className="play-scene__composer" id="play-intent-composer"><div className="play-scene__composer-heading"><div><p className="play-scene__eyebrow">{copy(language, "DECLARE YOUR INTENT", "ประกาศเจตนาของเจ้า")}</p><h2>{copy(language, "What will you do?", "เจ้าจะทำอย่างไร")}</h2></div>{!useLocal && !isAuthenticated && <button className="play-scene__login" onClick={onLogin}>{copy(language, "AI ASSISTANCE", "ใช้ AI-assisted")}</button>}</div>
        ...
        {diceResult && <DiceResult record={diceResult} language={language} momentum={game.character.vitals.momentum} onSpend={spendMomentum} onAccept={acceptResult} onEdit={resetIntent} rolling={rolling} displayDice={displayDice} />}
      </section>
    </section>}
  </div>;
}
