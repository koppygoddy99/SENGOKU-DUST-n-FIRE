...
function gmContext(game: GameState) {
  ...
  return {
    ...
    character: { name: game.character.name, occupation: game.character.occupation, origin: game.character.origin, strengths: game.character.strength, weakness: game.character.weakness, attributes: game.character.attributes, masteries: game.character.masteries.map((entry) => ({ name: entry.label, level: entry.level, source: entry.origin })), background: game.character.pulls.filter((entry) => entry.answer !== "ยังไม่บอก" && entry.answer !== "ยังไม่ตอบ").slice(0, 2).map((entry) => ({ question: entry.question, answer: entry.answer, tags: entry.tags })) },
