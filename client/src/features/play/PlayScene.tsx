...
import { STATS, applyMomentumFromSource, applyRoll, getMomentumSources, masteryLevelDetails, parseAction, resolveRoll, traitValueForRoll, xpNeededForMasteryLevel, type GameState, type MomentumSource, type RollPreview } from "@/lib/game";
...
function RollFormula({ record, game, language }: { record: OutcomeRecord; game: GameState; language: Language }) {
  ...
  return <section data-testid="roll-formula" className="play-dice-result__formula" aria-label={copy(language, "Roll calculation", "การคำนวณผลทอย")}>
    <div className="play-dice-result__formula-heading"><p className="play-scene__eyebrow">{copy(language, "HOW THIS RESULT WAS BUILT", "ผลนี้คำนวณจากอะไร")}</p><strong>{bypassesRoll ? copy(language, "SPECIAL ITEM → PASS / DN 0", "ไอเทมเฉพาะทาง → ผ่าน / DN 0") : `${diceTotal} + ${statValue} + ${masteryValue} + ${record.contextBonus}${record.flawTriggered ? " − 2" : ""} + ${record.momentumSpent} = ${record.total}`}</strong></div>
...
function RollDetails({ preview, game, language, momentum, onRoll, onEdit, rolling }: { preview: RollPreview; game: GameState; language: Language; momentum: number; onRoll: () => void; onEdit: () => void; rolling: boolean }) {
  ...
  return <section className="play-roll-details"><div className="play-roll-details__heading"><span><SengokuIcon name="sword" tone="vermilion" size={17} /> {copy(language, "ROLL DETAILS", "รายละเอียดการทอย")}</span><small>{copy(language, "Rules are visible after intent", "เห็นกติกาหลังยืนยันเจตนา")}</small></div><div className="play-roll-details__grid"><div><small>{copy(language, "Intent", "เจตนา")}</small><strong>{preview.intent}</strong></div><div><small>{copy(language, "Trait", "คุณลักษณะ")}</small><strong>{language === "en" ? stat?.en : stat?.th} · +{traitValue}</strong><span>{copy(language, "Raw Trait value is added directly by the engine.", "engine จะบวกค่า Trait จริงโดยตรง")}</span></div><div><small>{copy(language, "Mastery", "ความชำนาญ")}</small><strong>{preview.mastery ? `${preview.mastery.label} · ${copy(language, "Level", "ระดับ")} ${preview.mastery.level} · +${preview.mastery.level}` : copy(language, "No mastery", "ไม่มีความชำนาญ")}</strong></div><div><small>{copy(language, "Context / Gear", "บริบท / สัมภาระ")}</small><strong>{preview.contextReason ? `${preview.contextReason} +${preview.contextBonus}` : copy(language, "No bonus", "ไม่มีโบนัส")}</strong><span>{copy(language, "Maximum +2", "สูงสุด +2")}</span></div><div><small>{preview.specialItem ? copy(language, "Special item", "ไอเทมเฉพาะทาง") : copy(language, "Difficulty", "ความยาก")}</small><strong>{preview.specialItem ? `${preview.specialItem.label} · ${copy(language, "PASS", "ผ่านเลย")}` : `DN ${preview.difficulty}`}</strong><span>{preview.difficultyReason}</span></div><div><small>{copy(language, "Momentum", "แรงฮึด")}</small><strong>{momentum}/2 · {copy(language, "available after roll", "เลือกใช้หลังเห็นผล")}</strong></div></div>{preview.historical && <HistoricalFence historical={preview.historical} language={language} />}<div className="play-roll-details__actions"><Button className="df-button df-button--ghost" onClick={onEdit} disabled={rolling}>{copy(language, "CHANGE INTENT", "แก้เจตนา")}</Button><Button className="df-button df-button--primary" onClick={onRoll} disabled={rolling}>{rolling ? copy(language, "ROLLING…", "กำลังทอย…") : preview.specialItem ? copy(language, "PASS WITH ITEM", "ผ่านด้วยไอเทม") : copy(language, "ROLL 2D12", "ทอย 2D12")} <ArrowRight size={17} /></Button></div></section>;
}
...
function DiceResult({ record, game, language, momentum, momentumSources, selectedMomentumSourceId, onSelectMomentumSource, onSpend, onAccept, onEdit, rolling, rollPhase, displayDice }: { record: OutcomeRecord; game: GameState; language: Language; momentum: number; momentumSources: MomentumSource[]; selectedMomentumSourceId: string; onSelectMomentumSource: (sourceId: string) => void; onSpend: () => void; onAccept: () => void; onEdit: () => void; rolling: boolean; rollPhase: "rolling" | "settling" | null; displayDice: [number, number] }) {
  ...
  const canSpendMomentum = momentumSources.length > 0 && !record.momentumSpent && !bypassesRoll;
  ...
  return <div className="page play-scene-page">
    ...
    <section className="play-scene__paper">
      {outcome ? outcomePresentation === "revealed" ? <OutcomeCard record={outcome} game={game} language={language} notice={notice} onContinue={focusComposer} onMap={() => onOpen("home")} onChronicle={() => onOpen("log")} onUseSuggestion={(suggestion) => { setAction(suggestion); focusComposer(); }} /> : <OutcomeDraft record={outcome} language={language} pending={narrationPending} notice={notice} onReady={() => setOutcomePresentation("revealed")} /> : <>
        ...
        <section className="play-scene__skill-ledger" aria-label={copy(language, "Mastery progress", "ความก้าวหน้าของวิชา")}><p className="play-scene__eyebrow">{copy(language, "MASTERY LEDGER", "สมุดความชำนาญ")}</p><div>{game.character.masteries.map((mastery) => { const masteryDetails = masteryLevelDetails(mastery.level); const needed = xpNeededForMasteryLevel(mastery.level); return <article key={mastery.id} className={details?.mastery?.id === mastery.id ? "is-selected" : ""}><small>{mastery.label}</small><strong>{copy(language, "Level", "ระดับ")} {mastery.level} · +{mastery.level} · {language === "en" ? masteryDetails.en : masteryDetails.th}</strong><span>{needed === 0 ? copy(language, "Peerless", "หาตัวจับไม่ได้") : `${mastery.xp ?? 0}/${needed} Progress`}</span></article>; })}</div></section>
      ...
      {(!outcome || outcomePresentation === "revealed") && <section className="play-scene__composer" id="play-intent-composer"><div className="play-scene__composer-heading"><div><p className="play-scene__eyebrow">{copy(language, "DECLARE YOUR INTENT", "ประกาศเจตนาของเจ้า")}</p><h2>{copy(language, "What will you do?", "เจ้าจะทำอย่างไร")}</h2></div>{!useLocal && !isAuthenticated && <button className="play-scene__login" onClick={onLogin}>{copy(language, "AI ASSISTANCE", "ใช้ AI-assisted")}</button>}</div>
        ...
        {details && !diceResult && <RollDetails preview={details} game={game} language={language} momentum={game.character.vitals.momentum} onRoll={roll} onEdit={resetIntent} rolling={resolveGM.isPending || rolling} />}
