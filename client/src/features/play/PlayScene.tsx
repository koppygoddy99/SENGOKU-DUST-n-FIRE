...
import { acceptRandomEventQuest, rejectRandomEventQuest } from "@/lib/randomEvents";
...
const { STATS, activeMainMission, applyMissionDirective, applyRoll, mergeAIAnalysis, masteryLevelDetails, parseAction, resolveRoll, traitLevelDetails, traitProgressNeededForLevel, traitValueForRoll, visibleSideLeads, xpNeededForMasteryLevel } = game;
...
export function PlayScene({ game, language, onOpen, onUpdate, isAuthenticated, uiPreviewMode, onLogin, onAccountCreditChange }: { game: GameState; language: Language; onOpen: (page: PlayDestination) => void; onUpdate: (next: GameState, message: string) => void; isAuthenticated: boolean; uiPreviewMode: boolean; onLogin: () => void; onAccountCreditChange: () => unknown }) {
  ...
  const commitIntent = () => {
    ...
    analyzeGM.mutate({ action, language, context: gmContext(game) }, { onSuccess: (answer) => { const fallback = parseAction(action, game); setDetails(mergeAIAnalysis(fallback, answer, game.character.masteries)); setNotice(copy(language, "AI-assisted interpretation is ready. You may inspect or revise it before rolling.", "คำวินิจฉัยแบบ AI-assisted พร้อมแล้ว ตรวจหรือแก้ได้ก่อนทอย")); }, onError: () => { local(); setNotice(copy(language, "AI assistance is unavailable. The story continues with local rules and Local Save.", "AI ยังไม่พร้อม เรื่องยังดำเนินด้วยกติกาในเครื่องและ Local Save")); } });
  };
  ...
  return <div className="page play-scene-page">
    {game.pendingRandomEvent ? <RandomEventModal game={game} onAccept={(choiceId) => { const next = acceptRandomEventQuest(game, choiceId); onUpdate({ ...next, credits: game.credits }, copy(language, "Event quest accepted — resolve it with your own rolls", "รับเควสเหตุการณ์แล้ว — ใช้การทอยของเจ้าปิดมันเอง")); }} onReject={() => { const next = rejectRandomEventQuest(game); onUpdate({ ...next, credits: game.credits }, copy(language, "You walked past the event", "เจ้าเดินผ่านเหตุการณ์นี้ไป")); }} /> : null}
