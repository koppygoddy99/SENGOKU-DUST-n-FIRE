...
export function PlayScene({ game, language, onOpen, onUpdate, isAuthenticated, uiPreviewMode, onLogin, onAccountCreditChange }: { game: GameState; language: Language; onOpen: (page: PlayDestination) => void; onUpdate: (next: GameState, message: string) => void; isAuthenticated: boolean; uiPreviewMode: boolean; onLogin: () => void; onAccountCreditChange: () => unknown }) {
  ...
  const resetIntent = () => { setRisk(null); setDetails(null); setDiceResult(null); setRolling(false); setNotice(""); };
  ...
  const roll = () => { if (!details) return; const record = resolveRoll(details, game, false); setDiceResult(record); const reduced = window.matchMedia?.("(prefers-reduced-motion: reduce)").matches ?? true; if (reduced) { setDisplayDice(record.dice); setNotice(copy(language, "The dice are visible. Momentum may be spent now, before this result is recorded.", "เห็นผลลูกเต๋าแล้ว จะใช้แรงฮึดได้ตอนนี้ก่อนบันทึกผล")); return; } setRolling(true); const interval = window.setInterval(() => setDisplayDice([Math.floor(Math.random() * 12) + 1, Math.floor(Math.random() * 12) + 1]), 90); window.setTimeout(() => { window.clearInterval(interval); setDisplayDice(record.dice); setRolling(false); setNotice(copy(language, "The dice are visible. Momentum may be spent now, before this result is recorded.", "เห็นผลลูกเต๋าแล้ว จะใช้แรงฮึดได้ตอนนี้ก่อนบันทึกผล")); }, 720); };
