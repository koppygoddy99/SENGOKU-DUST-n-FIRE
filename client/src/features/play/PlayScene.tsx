...
function RollDetails({ preview, game, language, onRoll, onEdit, rolling }: { preview: RollPreview; game: GameState; language: Language; onRoll: () => void; onEdit: () => void; rolling: boolean }) {
  ...
  return <section className="play-roll-details"><div className="play-roll-details__heading"><span><SengokuIcon name="sword" tone="vermilion" size={17} /> {copy(language, "ROLL DETAILS", "รายละเอียดการทอย")}</span><small>{copy(language, "Rules are visible after intent", "เห็นกติกาหลังยืนยันเจตนา")}</small></div><div className="play-roll-details__grid"><div><small>{copy(language, "Intent", "เจตนา")}</small><strong>{preview.intent}</strong></div><div><small>{copy(language, "Trait", "คุณลักษณะ")}</small><strong>{language === "en" ? stat?.en : stat?.th} · {copy(language, "Level", "ระดับ")} {traitValue} · +{traitValue}</strong><span>{copy(language, "Raw Trait value is added directly by the engine.", "engine จะบวกค่า Trait จริงโดยตรง")}</span></div><div><small>{copy(language, "Trait Progress", "ความก้าวหน้า Trait")}</small><strong>{language === "en" ? details.en : details.th}</strong><span>{needed === 0 ? copy(language, "Level 10 reached", "ถึง Level 10 แล้ว") : `${progress?.xp ?? 0}/${needed} Progress`}</span></div><div><small>{copy(language, "Mastery", "ความชำนาญ")}</small><strong>{preview.mastery ? `${preview.mastery.label} · ${copy(language, "Level", "ระดับ")} ${preview.mastery.level} · +${preview.mastery.level}` : copy(language, "No mastery", "ไม่มีความชำนาญ")}</strong></div><div><small>{copy(language, "Context / Gear", "บริบท / สัมภาระ")}</small><strong>{preview.contextReason ? `${preview.contextReason} +${preview.contextBonus}` : copy(language, "No bonus", "ไม่มีโบนัส")}</strong><span>{copy(language, "Maximum +2", "สูงสุด +2")}</span></div><div><small>{preview.specialItem ? copy(language, "Special item", "ไอเทมเฉพาะทาง") : copy(language, "Difficulty", "ความยาก")}</small><strong>{preview.specialItem ? `${preview.specialItem.label} · ${copy(language, "PASS", "ผ่านเลย")}` : `DN ${preview.difficulty}`}</strong><span>{preview.difficultyReason}</span></div></div>{preview.historical && <HistoricalFence historical={preview.historical} language={language} />}<div className="play-roll-details__actions"><Button className="df-button df-button--ghost" onClick={onEdit} disabled={rolling}>{copy(language, "CHANGE INTENT", "แก้เจตนา")}</Button><Button className="df-button df-button--primary" onClick={onRoll} disabled={rolling}>{rolling ? copy(language, "ROLLING…", "กำลังทอย…") : preview.specialItem ? copy(language, "PASS WITH ITEM", "ผ่านด้วยไอเทม") : copy(language, "ROLL 2D12", "ทอย 2D12")} <ArrowRight size={17} /></Button></div></section>;
}
