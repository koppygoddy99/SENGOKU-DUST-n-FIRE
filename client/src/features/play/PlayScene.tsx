...
import { AXES, applyMomentumToRoll, applyRoll, masteryTierForRank, parseAction, resolveRoll, xpNeededForRank, type GameState, type RollPreview } from "@/lib/game";
...
function RollFormula({ record, game, language }: { record: OutcomeRecord; game: GameState; language: Language }) {
  ...
  const parts = [
    ...
    { label: language === "en" ? axis?.en ?? "Axis" : axis?.th ?? "แกน" , value: `${axisValue >= 0 ? "+" : ""}${axisValue}`, note: copy(language, "Attribute used for this approach", "ค่าสถานะที่ใช้กับวิธีนี้") },
    ...
    { label: copy(language, "MOMENTUM", "แรงฮึด"), value: `${record.momentumSpent >= 0 ? "+" : ""}${record.momentumSpent}`, note: record.momentumSpent ? copy(language, "Spent after seeing the dice", "ใช้หลังเห็นผลลูกเต๋า") : copy(language, "Not spent", "ยังไม่ได้ใช้") },
  ];
  return <section data-testid="roll-formula" className="play-dice-result__formula" aria-label={copy(language, "Roll calculation", "การคำนวณผลทอย")}>
    <div className="play-dice-result__formula-heading"><p className="play-scene__eyebrow">{copy(language, "HOW THIS RESULT WAS BUILT", "ผลนี้คำนวณจากอะไร")}</p><strong>{diceTotal} {axisValue >= 0 ? "+" : ""}{axisValue} {masteryValue >= 0 ? "+" : ""}{masteryValue} {record.contextBonus >= 0 ? "+" : ""}{record.contextBonus} {record.momentumSpent >= 0 ? "+" : ""}{record.momentumSpent} = {record.total}</strong></div>
...
function DiceResult({ record, game, language, momentum, onSpend, onAccept, onEdit, rolling, rollPhase, displayDice }: { record: OutcomeRecord; game: GameState; language: Language; momentum: number; onSpend: () => void; onAccept: () => void; onEdit: () => void; rolling: boolean; rollPhase: "rolling" | "settling" | null; displayDice: [number, number] }) {
...
export function PlayScene({ game, language, onOpen, onUpdate, isAuthenticated, uiPreviewMode, onLogin, onAccountCreditChange }: { game: GameState; language: Language; onOpen: (page: PlayDestination) => void; onUpdate: (next: GameState, message: string) => void; isAuthenticated: boolean; uiPreviewMode: boolean; onLogin: () => void; onAccountCreditChange: () => unknown }) {
  ...
  const spendMomentum = () => { if (!diceResult || game.character.vitals.momentum <= 0) return; const record = applyMomentumToRoll(diceResult, game); setDisplayDice(record.dice); setDiceResult(record); };
  ...
  return <div className="page play-scene-page">
    ...
    <section className="play-scene__paper">
      ...
      {(!outcome || outcomePresentation === "revealed") && <section className="play-scene__composer" id="play-intent-composer"><div className="play-scene__composer-heading"><div><p className="play-scene__eyebrow">{copy(language, "DECLARE YOUR INTENT", "ประกาศเจตนาของเจ้า")}</p><h2>{copy(language, "What will you do?", "เจ้าจะทำอย่างไร")}</h2></div>{!useLocal && !isAuthenticated && <button className="play-scene__login" onClick={onLogin}>{copy(language, "AI ASSISTANCE", "ใช้ AI-assisted")}</button>}</div>
        ...
        {diceResult && <DiceResult record={diceResult} game={game} language={language} momentum={game.character.vitals.momentum} onSpend={spendMomentum} onAccept={acceptResult} onEdit={resetIntent} rolling={rolling} rollPhase={rollPhase} displayDice={displayDice} />}
      </section>}
    </section>
  </div>;
}
