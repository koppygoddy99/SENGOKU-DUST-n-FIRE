...
import { AXES, applyRoll, masteryTierForRank, parseAction, resolveRoll, xpNeededForRank, type GameState, type RollPreview } from "@/lib/game";
...
function RollDetails({ preview, language, momentum, onRoll, onEdit, rolling }: { preview: RollPreview; language: Language; momentum: number; onRoll: () => void; onEdit: () => void; rolling: boolean }) {
  ...
  return <section className="play-roll-details"><div className="play-roll-details__heading"><span><SengokuIcon name="sword" tone="vermilion" size={17} /> {copy(language, "ROLL DETAILS", "รายละเอียดการทอย")}</span><small>{copy(language, "Rules are visible after intent", "เห็นกติกาหลังยืนยันเจตนา")}</small></div><div className="play-roll-details__grid"><div><small>{copy(language, "Intent", "เจตนา")}</small><strong>{preview.intent}</strong></div><div><small>{copy(language, "Axis", "แกนทอย")}</small><strong>{language === "en" ? axis?.en : axis?.th}</strong></div><div><small>{copy(language, "Mastery", "ความชำนาญ")}</small><strong>{preview.mastery ? `${preview.mastery.label} · ${copy(language, "Step", "ขั้น")} ${preview.mastery.rank ?? 1} · +${tier?.bonus}` : copy(language, "No mastery", "ไม่มีความชำนาญ")}</strong></div><div><small>{copy(language, "Context", "บริบท")}</small><strong>{preview.contextReason ? `${preview.contextReason} +${preview.contextBonus}` : copy(language, "No bonus", "ไม่มีโบนัส")}</strong></div><div><small>{copy(language, "Difficulty", "ความยาก")}</small><strong>DN {preview.difficulty}{tier && preview.difficulty < tier.minimumDifficulty ? ` · ${copy(language, "practice will not grow", "ยังไม่พอให้วิชาเติบโต")}` : ""}</strong></div><div><small>{copy(language, "Momentum", "แรงฮึด")}</small><strong>{momentum}/2 · {copy(language, "available after roll", "เลือกใช้หลังเห็นผล")}</strong></div></div>{preview.historical && <HistoricalFence historical={preview.historical} language={language} />}<div className="play-roll-details__actions"><Button className="df-button df-button--ghost" onClick={onEdit} disabled={rolling}>{copy(language, "CHANGE INTENT", "แก้เจตนา")}</Button><Button className="df-button df-button--primary" onClick={onRoll} disabled={rolling}>{rolling ? copy(language, "ROLLING…", "กำลังทอย…") : copy(language, "ROLL 2D12", "ทอย 2D12")} <ArrowRight size={17} /></Button></div></section>;
}

function DiceResult({ record, language, momentum, onSpend, onAccept, onEdit, rolling, rollPhase, displayDice }: { record: OutcomeRecord; language: Language; momentum: number; onSpend: () => void; onAccept: () => void; onEdit: () => void; rolling: boolean; rollPhase: "rolling" | "settling" | null; displayDice: [number, number] }) {
  const canSpendMomentum = momentum > 0 && !record.momentumSpent;
  const diceStatus = rolling ? (rollPhase === "settling" ? copy(language, "settling", "กำลังหยุด") : copy(language, "rolling", "กำลังทอย")) : outcomeLabel(record.outcome);
  return <section data-testid="dice-decision-window" className={`play-dice-result ${rolling ? "is-rolling" : ""} ${rollPhase === "settling" ? "is-settling" : ""}`} aria-live="polite"><p className="play-scene__eyebrow">{rolling ? copy(language, "THE ROOM HOLDS ITS BREATH", "ทั้งห้องกลั้นลมหายใจ") : copy(language, "DICE RESULT · DECISION WINDOW", "ผลลูกเต๋า · ช่วงตัดสินใจ")}</p><div className="play-dice-result__tray"><span data-testid="dice-one" aria-label={copy(language, `First die: ${displayDice[0]}`, `ลูกเต๋าลูกแรก: ${displayDice[0]}`)}>{displayDice[0]}</span><b>+</b><span data-testid="dice-two" className="is-light" aria-label={copy(language, `Second die: ${displayDice[1]}`, `ลูกเต๋าลูกที่สอง: ${displayDice[1]}`)}>{displayDice[1]}</span><div><small>{copy(language, "CURRENT TOTAL", "ผลรวมปัจจุบัน")}</small><strong>{rolling ? "…" : record.total} <i>/ DN {record.difficulty}</i></strong></div><em>{diceStatus}</em></div>{!rolling && <><div className="play-dice-result__copy"><p>{record.summary}</p><p>{record.consequence}</p></div>{canSpendMomentum && <div className="play-dice-result__momentum"><Sparkles size={17} /><div><strong>{copy(language, "Spend 1 Momentum for +2?", "ใช้แรงฮึด 1 เพื่อ +2 หรือไม่?")}</strong><span>{copy(language, "You see the dice first. Spending it will recalculate the same deterministic roll before the result is saved.", "เจ้าเห็นผลลูกเต๋าก่อน การใช้แรงฮึดจะคำนวณผลทอยเดิมใหม่ก่อนบันทึก")}</span></div><Button className="df-button df-button--ghost" onClick={onSpend}>{copy(language, "SPEND MOMENTUM", "ใช้แรงฮึด")}</Button></div>}<div className="play-dice-result__actions"><Button className="df-button df-button--ghost" onClick={onEdit}><ArrowLeft size={16} /> {copy(language, "CHANGE INTENT", "แก้เจตนา")}</Button><Button className="df-button df-button--primary" onClick={onAccept}>{copy(language, "RECORD THIS RESULT", "บันทึกผลนี้")} <Check size={16} /></Button></div></>}</section>;
}

function TypewriterProse({ text }: { text: string }) {
  ...
  return <p className="play-outcome-card__typed" aria-live="polite" aria-busy={visible.length < text.length}>{visible}{visible.length < text.length && <span aria-hidden="true">▌</span>}</p>;
}
...
function OutcomeCard({ record, game, language, notice, onContinue, onMap, onChronicle }: { record: OutcomeRecord; game: GameState; language: Language; notice: string; onContinue: () => void; onMap: () => void; onChronicle: () => void }) {
  ...
  return <section data-testid="narrative-outcome" className="play-outcome-card"><header><div><p className="play-scene__eyebrow">{copy(language, "OUTCOME · SAVED TO CAMPAIGN", "ผลลัพธ์ · บันทึกลงแคมเปญแล้ว")}</p><h2>{outcomeLabel(record.outcome)}</h2></div><span>{copy(language, "Leaf", "หน้า")} {game.progression?.leaf ?? record.tick}</span></header>{notice && <p className="play-scene__notice play-outcome-card__notice">{notice}</p>}<article className="play-outcome-card__prose"><small>{copy(language, "STORY RESULT · WHAT FOLLOWS", "ผลเชิงเรื่องเล่า · สิ่งที่เกิดต่อ")}</small><strong>{game.currentScene.title}</strong><TypewriterProse text={narrative} /><p>{worldTrace?.detail ?? record.consequence}</p></article><div className="play-outcome-card__layers"><article><small>{copy(language, "1 · ROLL RESULT", "1 · ผลทอย")}</small><strong>{record.total} / DN {record.difficulty}</strong><p>{record.summary}</p></article><article><small>{copy(language, "2 · SKILL PRACTICE", "2 · วิชาที่ฝึก")}</small><strong>{practice ? `${practice.masteryLabel} ${practice.gained ? `+${practice.gained} XP` : copy(language, "held", "ยังไม่ขยับ")}` : copy(language, "No new practice", "ไม่มีวิชาใหม่")}</strong><p>{practice ? `${copy(language, "Step", "ขั้น")} ${practice.rankAfter} · ${practice.masteryMark ?? `${practice.xp}/${practice.xpNeeded} XP`} · ${practice.note ?? ""}` : ""}</p></article><article><small>{copy(language, "3 · TIME & THREAD", "3 · เวลาและเส้นเรื่อง")}</small><strong>{timeMark?.message ?? copy(language, "The scene holds", "ฉากยังคงอยู่")}</strong><p>{missionUpdate ? `${missionUpdate.current}/${missionUpdate.required} · ${missionUpdate.state}${missionUpdate.reward ? ` · ${missionUpdate.reward}` : ""}` : copy(language, "No mission change", "ภารกิจยังไม่เปลี่ยน")}</p></article></div><div className="play-outcome-card__actions"><Button className="df-button df-button--primary" onClick={onContinue}>{copy(language, "WRITE NEXT INTENT", "เขียนเจตนาถัดไป")} <ArrowRight size={17} /></Button><Button className="df-button df-button--ghost" onClick={onMap}><Map size={16} /> {copy(language, "RETURN TO MAP", "กลับแผนที่")}</Button><button onClick={onChronicle}><FileText size={15} /> {copy(language, "OPEN CHRONICLE", "เปิดบันทึกเรื่อง")}</button></div></section>;
}
...
export function PlayScene({ game, language, onOpen, onUpdate, isAuthenticated, uiPreviewMode, onLogin, onAccountCreditChange }: { game: GameState; language: Language; onOpen: (page: PlayDestination) => void; onUpdate: (next: GameState, message: string) => void; isAuthenticated: boolean; uiPreviewMode: boolean; onLogin: () => void; onAccountCreditChange: () => unknown }) {
  ...
  const roll = () => { if (!details) return; clearDiceMotion(); const record = resolveRoll(details, game, false); setDiceResult(record); const reduced = window.matchMedia?.("(prefers-reduced-motion: reduce)").matches ?? true; if (reduced) { setDisplayDice(record.dice); setRollPhase(null); setNotice(copy(language, "The dice are visible. Momentum may be spent now, before this result is recorded.", "เห็นผลลูกเต๋าแล้ว จะใช้แรงฮึดได้ตอนนี้ก่อนบันทึกผล")); return; } setRolling(true); setRollPhase("rolling"); diceTimers.current.interval = window.setInterval(() => setDisplayDice([Math.floor(Math.random() * 12) + 1, Math.floor(Math.random() * 12) + 1]), 72); diceTimers.current.settle = window.setTimeout(() => { if (diceTimers.current.interval) window.clearInterval(diceTimers.current.interval); setDisplayDice(record.dice); setRollPhase("settling"); }, ROLL_ANIMATION_MS - 650); diceTimers.current.finish = window.setTimeout(() => { clearDiceMotion(); setDisplayDice(record.dice); setRolling(false); setRollPhase(null); setNotice(copy(language, "The dice are visible. Momentum may be spent now, before this result is recorded.", "เห็นผลลูกเต๋าแล้ว จะใช้แรงฮึดได้ตอนนี้ก่อนบันทึกผล")); }, ROLL_ANIMATION_MS); };
  const spendMomentum = () => { if (!details || game.character.vitals.momentum <= 0) return; const record = resolveRoll(details, game, true); setDisplayDice(record.dice); setDiceResult(record); };
  const saveLocal = (record: OutcomeRecord, message: string) => { const next = applyRoll(game, record); onUpdate({ ...next, credits: game.credits }, message); setOutcome(record); setDiceResult(null); setDetails(null); setAction(""); setNotice(message); };
  const acceptResult = () => {
  ...
  return <div className="page play-scene-page">
    ...
    <section className="play-scene__paper">
      ...
      <section className="play-scene__skill-ledger" aria-label={copy(language, "Skill practice", "วิชาที่ฝึก")}><p className="play-scene__eyebrow">{copy(language, "SKILL LEDGER", "สมุดวิชาที่ฝึก")}</p><div>{game.character.masteries.map((mastery) => { const rank = mastery.rank ?? 1; const tier = masteryTierForRank(rank); return <article key={mastery.id} className={details?.mastery?.id === mastery.id ? "is-selected" : ""}><small>{mastery.label}</small><strong>{copy(language, "Step", "ขั้น")} {rank} · +{tier.bonus} · {language === "en" ? tier.en : tier.th}</strong><span>{rank === 20 ? copy(language, "Mastered", "ถึงขีดสุด") : `${mastery.xp ?? 0}/${xpNeededForRank(rank)} XP · DN ${tier.minimumDifficulty}+`}</span></article>; })}</div></section>
      ...
      <section className="play-scene__composer" id="play-intent-composer"><div className="play-scene__composer-heading"><div><p className="play-scene__eyebrow">{copy(language, "DECLARE YOUR INTENT", "ประกาศเจตนาของเจ้า")}</p><h2>{copy(language, "What will you do?", "เจ้าจะทำอย่างไร")}</h2></div>{!useLocal && !isAuthenticated && <button className="play-scene__login" onClick={onLogin}>{copy(language, "AI ASSISTANCE", "ใช้ AI-assisted")}</button>}</div>
        ...
        {diceResult && <DiceResult record={diceResult} language={language} momentum={game.character.vitals.momentum} onSpend={spendMomentum} onAccept={acceptResult} onEdit={resetIntent} rolling={rolling} rollPhase={rollPhase} displayDice={displayDice} />}
      </section>
    </section>
  </div>;
}
