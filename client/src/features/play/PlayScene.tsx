...
import { STATS, applyMomentumFromSource, applyRoll, bonusForStatValue, getMomentumSources, masteryTierForRank, parseAction, resolveRoll, xpNeededForRank, type GameState, type MomentumSource, type RollPreview } from "@/lib/game";
...
function RollFormula({ record, game, language }: { record: OutcomeRecord; game: GameState; language: Language }) {
  ...
  const statValue = bonusForStatValue(game.character.attributes[record.stat]);
  ...
  return <section data-testid="roll-formula" className="play-dice-result__formula" aria-label={copy(language, "Roll calculation", "การคำนวณผลทอย")}>
    <div className="play-dice-result__formula-heading"><p className="play-scene__eyebrow">{copy(language, "HOW THIS RESULT WAS BUILT", "ผลนี้คำนวณจากอะไร")}</p><strong>{diceTotal} {statValue >= 0 ? "+" : ""}{statValue} {masteryValue >= 0 ? "+" : ""}{masteryValue} {record.contextBonus >= 0 ? "+" : ""}{record.contextBonus}{record.flawTriggered ? " − 2" : ""} {record.momentumSpent >= 0 ? "+" : ""}{record.momentumSpent} = {record.total}</strong></div>
