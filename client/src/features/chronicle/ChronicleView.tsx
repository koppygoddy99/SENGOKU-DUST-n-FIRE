...
export function ChronicleView({ game, language, readerMode, setReaderMode }: { game: GameState; language: Language; readerMode: boolean; setReaderMode: (value: boolean) => void }) {
  ...
  return <div className={`page log-view chronicle-view ${readerMode ? "is-reader" : ""}`}>
    ...
    {readerMode ? <section className="reader-paper"><div className="reader-paper__chapter">{game.campaign.title} · {copy(language, "LEAF", "หน้า")} {readerEntry?.tick ?? 1} · {season} · {copy(language, "Day", "วันที่")} {game.campaign.day}</div><h2>{readerEntry?.title ?? copy(language, "The first page", "หน้าแรก")}</h2><div className="reader-paper__scroll">{splitStoryParagraphs(readerStory).map((paragraph: string, index: number) => <p key={`${readerEntry?.id ?? "empty"}-${index}`}>{paragraph}</p>)}<p className="reader-question">{game.currentScene.prompt}</p></div><div className="reader-paper__footer"><button disabled={readerIndex <= 0} onClick={() => setReaderIndex((index) => Math.max(0, index - 1))}><ArrowLeft size={16} /> {copy(language, "Previous", "ก่อนหน้า")}</button><span>{readerIndex + 1} / {Math.max(entries.length, 1)}</span><button disabled={readerIndex >= entries.length - 1} onClick={() => setReaderIndex((index) => Math.min(entries.length - 1, index + 1))}>{copy(language, "Next", "ถัดไป")} <ArrowRight size={16} /></button></div></section> : <>
      <header className="chronicle-view__heading"><div><p className="chronicle-view__eyebrow">{copy(language, "INTERACTIVE NOVEL LIBRARY", "ห้องสมุดนิยายโต้ตอบ")}</p><h1>{copy(language, "Chronicle", "พงศาวดาร")}</h1><p>{copy(language, "Read the leaves this campaign earned. Mechanics remain available when you need to inspect a consequence.", "อ่านหน้าที่แคมเปญนี้สร้างไว้ รายละเอียดการทอยยังเปิดดูได้เมื่อต้องตรวจผลกระทบ")}</p></div><div className="chronicle-view__count"><BookOpen size={18} /><strong>{entries.length}</strong><span>{copy(language, "recorded leaves", "หน้าที่ถูกบันทึก")}</span></div></header>
