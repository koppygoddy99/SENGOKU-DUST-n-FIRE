...
describe("review screen manifest", () => {
  it("defines the complete 16-screen review set with unique routes, titles, seeds, and file names", () => {
    expect(REVIEW_SCREEN_MANIFEST).toHaveLength(16);
    expect(new Set(REVIEW_SCREEN_MANIFEST.map((screen) => screen.reviewQuery)).size).toBe(16);
    expect(new Set(REVIEW_SCREEN_MANIFEST.map((screen) => screen.screenshotFile)).size).toBe(16);
    expect(new Set(REVIEW_SCREEN_MANIFEST.map((screen) => screen.pageTitle)).size).toBe(16);
    expect(new Set(REVIEW_SCREEN_MANIFEST.map((screen) => screen.seed)).size).toBe(16);
