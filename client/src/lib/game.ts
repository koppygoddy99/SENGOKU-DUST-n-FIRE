...
function localSpeakerReply(speaker: string, issuerType: Mission["issuerType"] | undefined, name: string, failed: boolean) {
  const replies: Record<NonNullable<Mission["issuerType"]>, { failure: string; other: string }> = {
    merchant: {
      failure: `${speaker} ใช้นิ้วกดเชือกปอที่ขอบลังจนเส้นใยลั่นเบา ๆ เขาอ้าปากจะว่าอะไร แต่หันไปมองของที่เสียก่อน “ของเสียไปเท่าใด” เขาถาม “บอกข้าตรง ๆ เถอะ แล้วเจ้าช่วยตามกลับมาก่อนค่ำได้หรือไม่ ข้าไม่อยากเอาเรื่องนี้ไปบอกคนของข้าเอง”`,
      other: `${speaker} เลื่อนถุงผ้าออกจากขอบลังเพียงนิดเดียวแล้วถอนใจ “เอาเถอะ” เขาว่า “ข้าจ่ายค่าจ้างให้เจ้าได้ แต่บอกข้าก่อน ของจะออกจากท่านี้เมื่อไร ข้าจะได้จัดคนไม่ให้ยืนรอเก้อ”`,
    },
    samurai: {
      failure: `${speaker} วางมือบนปากฝักดาบโดยไม่จับด้าม เขามองรอยนั้นอยู่พักหนึ่งก่อนกล่าวว่า “รอยนี้ต้องมีคนตรวจอีกครั้ง” เขาเงยหน้าขึ้น “อย่าเพิ่งแตะของเดิม ไปเอามาให้ข้าดูก่อนตะวันตก แล้วเราค่อยว่ากัน”`,
      other: `${speaker} มอง${name}อยู่ครู่หนึ่งก่อนกล่าวว่า “งานนี้พอแล้ว” เขาหันไปดูคนที่ยืนรอข้างหลัง “เก็บของให้เรียบร้อย แล้วอยู่ที่นี่ก่อน หากมีคนถาม ข้าจะเป็นคนตอบเอง”`,
    },
    ruler: {
      failure: `${speaker} ไม่ยกเสียง เขารอให้คนทั้งฉากเงียบก่อนเอ่ยว่า “กลับไปเอาหลักฐานมา” เขาหยุดนิดหนึ่ง “ข้าจะฟังเมื่อของอยู่ต่อหน้า ไม่ใช่ก่อนนั้น”`,
      other: `${speaker} พยักหน้าเพียงครั้งเดียว “ไปได้” เขากล่าว แล้วมองตาม${name}อยู่ครู่หนึ่ง “เรื่องนี้ให้รายงานข้าก่อนพ้นคืนนี้ อย่าให้ข้าต้องส่งคนไปตาม”`,
    },
    temple: {
      failure: `${speaker} ลูบขอบผ้าห่มเก่าแล้วลดเสียงลง “ถ้าข่าวนี้ไปถึงคนทั้งลาน เด็กในวัดจะเดือดร้อนก่อนใคร” เขามองไปทางประตู “ไปหาทางหยุดมันเสียเถอะ ก่อนจะมีคนมาถามที่นี่”`,
      other: `${speaker} ประนมมืออยู่ครู่หนึ่งก่อนเอ่ยว่า “ของที่หายยังตามได้” เขาว่า “ถ้าเจ้าจะไป ก็ไปก่อนคนอื่นรู้ตัว ข้าจะให้คนเฝ้าประตูไว้”`,
    },
    commoner: {
      failure: `${speaker} กำชายเสื้อไว้แน่นแล้วพูดเบาจนแทบกลืนกับลมว่า “ถ้าเขามาถามถึงบ้าน ข้าจะตอบคนเดียวไม่ไหว” เขาเหลือบไปดูคนในเรือน “ช่วยหาคนที่เห็นเรื่องนี้ให้ข้าที อย่างน้อยให้ข้าไม่ต้องพูดอยู่คนเดียว”`,
      other: `${speaker} ถอนลมหายใจช้า ๆ แล้วเอ่ยว่า “วันนี้เราไปต่อได้” เขาว่า “พรุ่งนี้ถ้าเขากลับมาถาม เจ้าช่วยอยู่ด้วยสักครู่ได้ไหม ข้าไม่อยากให้เด็ก ๆ ได้ยินคนเดียว”`,
    },
  };
...
function localOutcomeNarration(preview: RollPreview, state: GameState, outcome: Outcome, consequence: string) {
  ...
  const first = `หลังจาก ${state.character.name} ลงมือทำตามที่ตั้งใจไว้ อากาศใน${state.currentScene.location}ก็เงียบลงชั่วขณะ เสียงไม้เก่าลั่นใต้ฝ่าเท้าและเสียงผ้ากระทบกันยังอยู่ แต่คนใกล้ที่สุดต่างถอยครึ่งก้าว ราวกับไม่อยากให้ชื่อของตนติดไปกับเรื่องนี้ ${state.currentScene.pressure}ยังไม่คลายตัว มันเพียงย้ายจากมุมมืดของฉากมาวางหนักบนบ่าของเขา จนทุกลมหายใจมีน้ำหนักขึ้น.`;
  const middle = `${localSpeakerReply(speaker, mission?.issuerType, state.character.name, outcome === "failure_with_consequence")} คนข้างหลังเขาหลบตา คนหนึ่งแสร้งก้มผูกเชือกรองเท้า ไม่มีใครอยากรับเป็นพยานให้สิ่งที่เกิดกับ${state.character.name} แต่ทุกคนได้ยินคำพูดนั้นครบถ้วน.`;
  const last = outcome === "decisive_success"
    ? `ทางที่เปิดขึ้นกว้างกว่าที่คนในฉากคาดไว้ แต่มิใช่ของขวัญเปล่า ๆ มันผูกชื่อของ${state.character.name}ไว้กับผู้คนใน${state.currentScene.location}แน่นขึ้นอีกเส้น เมื่อมือของ${speaker}ยอมขยับไปแตะสิ่งที่เคยปฏิเสธ ทุกคนก็เห็นตรงกันว่าทางนี้มีราคา และวันหนึ่งราคานั้นย่อมถูกทวงคืน.`
    : outcome === "failure_with_consequence"
      ? `เรื่องนี้ยังไม่จบ แต่ทางเดิมใช้ไม่ได้อีกต่อไป ก่อนจะขยับครั้งหน้า${state.character.name}ต้องยอมเสียบางอย่างเพื่อซื้อเวลา หรือปล่อยให้คนอื่นเรียบเรียงคำอธิบายแทน เมื่อทางผ่านด้านนอกถูกปิดลงทีละน้อย ไม่มีใครเอ่ยคำตัดสิน ทว่าทุกคนรู้ว่าจากนี้คำแก้ตัวจะมีราคาสูงกว่าเดิม.`
      : `สิ่งที่${state.character.name}ต้องการเกิดขึ้นเพียงส่วนหนึ่ง ส่วนราคาที่เหลือค้างอยู่ในฉากเหมือนรอยเท้าบนดินชื้นที่ฝนลบไม่ทัน มันจะตามไปเมื่อคนเริ่มเห็นผลประโยชน์ของตนในเรื่องเดียวกัน ${speaker}เก็บของช้า ๆ ราวกับให้เวลาสั้น ๆ แก่ทุกคนมองสิ่งที่เกิดขึ้น ก่อนหันไปยังทางหลักซึ่งยังเต็มไปด้วยหูที่พร้อมรับข่าวผิดเพี้ยน.`;
