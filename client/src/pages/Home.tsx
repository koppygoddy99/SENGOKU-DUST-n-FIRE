...
function StartView({ language, onStart }: { language: Language; onStart: (game: GameState) => void }) {
  ...
  const previewAttributes = template.attributes;
  ...
  return <div className="page start-view">
    ...
    <div className="start-layout"><ol className="step-rail">{["Campaign", "Starting path", "Character", "Confirmation"].map((entry, index) => <li key={entry} className={step === index + 1 ? "is-current" : step > index + 1 ? "is-done" : ""}><span>{index + 1}</span><div><strong>{entry}</strong><small>{["time and place", "one of ten paths", "ties and strengths", "local save begins"][index]}</small></div></li>)}</ol>
      <section className="start-form">
        ...
        {step === 3 && <><div className="field-grid field-grid--two"><label className="field-label">{label(language, "Character name", "ชื่อตัวละคร")}<input value={draft.name} onChange={(event) => updateDraft("name", event.target.value)} /></label><label className="field-label">{label(language, "Identity", "เพศหรืออัตลักษณ์")}<input value={draft.identity} onChange={(event) => updateDraft("identity", event.target.value)} /></label></div><label className="field-label field-label--spaced">{label(language, "Origin", "ที่มา")}<input value={draft.origin} onChange={(event) => updateDraft("origin", event.target.value)} /></label><div className="field-grid field-grid--two"><label className="field-label">{label(language, "Strength", "จุดเด่น")}<input value={draft.strength} onChange={(event) => updateDraft("strength", event.target.value)} /></label><label className="field-label">{label(language, "Weakness", "จุดด้อย")}<input value={draft.weakness} onChange={(event) => updateDraft("weakness", event.target.value)} /></label></div><div className="relation-questions">{RELATIONSHIP_QUESTIONS.map(([id, question]) => <label className="field-label" key={id}><span>{question}</span><input value={draft.answers[id] ?? ""} onChange={(event) => updateAnswer(id, event.target.value)} /></label>)}</div><div className="background-note"><SectionKicker>CHARACTER BACKGROUND</SectionKicker><strong>{label(language, "These answers do not change Stats or roll bonuses", "คำตอบสองข้อนี้ไม่เปลี่ยน Stat หรือโบนัสทอย")}</strong><p>{label(language, "They become remembered background. The GM may return to them indirectly as a rumor, pressure, person, place, or difficult choice when the story naturally allows it; they are never a guaranteed plot trigger.", "คำตอบจะกลายเป็นภูมิหลังที่โลกจดจำ GM อาจพากลับมาเกี่ยวข้องทางอ้อมเป็นข่าวลือ แรงกดดัน ผู้คน สถานที่ หรือทางเลือกยาก ๆ เมื่อเรื่องพาไปถึง โดยไม่ใช่จุดพลิกเรื่องที่ถูกบังคับให้เกิด")}</p></div><StepControls previous={() => setStep(2)} next={() => setStep(4)} /></>}
