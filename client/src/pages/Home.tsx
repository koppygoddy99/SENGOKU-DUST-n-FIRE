...
function PlayView({ game, language, open, onUpdate, isAuthenticated, uiPreviewMode, onLogin, onAccountCreditChange }: { game: GameState; language: Language; open: (page: PageId) => void; onUpdate: (game: GameState, message: string) => void; isAuthenticated: boolean; uiPreviewMode: boolean; onLogin: () => void; onAccountCreditChange: () => void }) {
  ...
  const resolve = () => {
    if (!preview || (!localRulesOnly && game.credits <= 0)) return;
    const record = resolveRoll(preview, game, spendMomentum);
    const resolved = applyRoll(game, record);
    const localCommit = (message = `${record.summary} · Local Trial saved at Leaf ${record.tick} · no AI credit used`) => { onUpdate(saveLocalTrialResult(resolved, game.credits), message); setPreview(null); setAction(""); setSpendMomentum(false); };
    ...
    resolveGM.mutate({ language, context: toGMContext(game), action, roll: { outcome: record.outcome, total: record.total, difficulty: record.difficulty, summary: record.summary, consequence: record.consequence ?? null } }, {
      onSuccess: (answer) => {
        const narratedRecord = { ...record, narrative: answer.narration.join("\n\n") };
        const narratedState = applyRoll(game, narratedRecord);
        const withNarration: GameState = {
          ...withHistoricalBoundary(narratedState, { status: answer.historicalStatus, fence: answer.historicalFence }),
          currentScene: { ...narratedState.currentScene, title: answer.sceneTitle, body: answer.narration, prompt: answer.missionNote, suggestedActions: answer.nextChoices },
          memories: [...narratedState.memories, { id: `gm-memory-${Date.now()}`, kind: "news", title: answer.memory.title, detail: answer.memory.detail, tone: answer.memory.tone, tick: narratedState.tick }, { id: `gm-history-${Date.now()}`, kind: "witness", title: label(language, `Historical boundary · ${historicalStatusLabel(answer.historicalStatus, "en")}`, `ขอบเขตประวัติศาสตร์ · ${historicalStatusLabel(answer.historicalStatus, "th")}`), detail: answer.historicalFence, tone: historicalTone(answer.historicalStatus), tick: narratedState.tick }],
        };
      ...
      onError: () => { setGmNote(label(language, "The AI GM was unavailable. Your Local Trial result was saved without spending an AI credit.", "AI GM ยังตอบไม่ได้ จึงบันทึกผลแบบกรอกทดลองไว้ในเครื่องโดยไม่หักเครดิต AI")); localCommit(gmUnavailableLocalTrialNotice(language, record.summary)); },
    });
  };
