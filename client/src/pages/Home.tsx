  ...
  return <div className={appClass}>
    ...
    <aside className={`sidebar ${menuOpen ? "sidebar--open" : ""} ${sidebarCollapsed ? "sidebar--collapsed" : ""}`}>
      ...
      <div className="sidebar__vitals">
        <Vital label={label(language, "Wounds", "บาดแผล")} value={`${game.character.vitals.wounds}/${game.character.vitals.maxWounds ?? 6}`} percent={(game.character.vitals.wounds / (game.character.vitals.maxWounds ?? 6)) * 100} tone="red" />
...
function CharacterView({ game, language, open }: { game: GameState; language: Language; open: (page: PageId) => void }) {
  ...
  return <div className="page character-view">
    ...
    <section className="dossier-header"><div className="dossier-name"><span className="mon-avatar mon-avatar--large">火</span><div><h2>{game.character.name}</h2><p>{game.character.strength}</p></div></div><div className="dossier-resources"><span>{label(language, "Wounds", "บาดแผล")}<strong>{game.character.vitals.wounds}/{game.character.vitals.maxWounds ?? 6}</strong></span><span>{label(language, "Focus", "ค่าสติ")}<strong>{game.character.vitals.focus}/{game.character.vitals.maxFocus ?? 6}</strong></span><span>{label(language, "Trait cap", "เพดาน Trait")}<strong>10</strong></span><span>{label(language, "Rank", "ยศ")}<strong>{game.character.social.rank}</strong></span></div></section>
