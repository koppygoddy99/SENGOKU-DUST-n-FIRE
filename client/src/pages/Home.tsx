...
const seedDraft: CharacterDraft = { name: "ผู้ไร้นาม", identity: "ผู้เล่นกำหนด", templateId: "ronin", freeformOccupation: "", origin: "เส้นทางชายแดน", strength: "อ่านทางหนีและอันตรายได้ไว", weakness: "มีหนี้ที่ยังไม่กล้าพูดถึง", answers: {}, attributePullIds: [] };
...
function StartView({ language, onStart }: { language: Language; onStart: (game: GameState) => void }) {
  ...
  const activePullIds = selectedAttributePullIds(draft);
  const previewAttributes = attributesForDraft(template, draft);
  ...
  const toggleAttributePull = (id: string) => setDraft((current) => {
    const selected = selectedAttributePullIds(current);
    const next = selected.includes(id) ? selected.filter((entry) => entry !== id) : [...selected, id].slice(-2);
    return { ...current, attributePullIds: next };
  });
  ...
  return <div className="page start-view">
    ...
    <div className="start-layout"><ol className="step-rail">{["Campaign", "Starting path", "Character", "Confirmation"].map((entry, index) => <li key={entry} className={step === index + 1 ? "is-current" : step > index + 1 ? "is-done" : ""}><span>{index + 1}</span><div><strong>{entry}</strong><small>{["time and place", "one of ten paths", "ties and strengths", "local save begins"][index]}</small></div></li>)}</ol>
      <section className="start-form">
        ...
        {step === 3 && <><div className="field-grid field-grid--two"><label className="field-label">{label(language, "Character name", "ชื่อตัวละคร")}<input value={draft.name} onChange={(event) => updateDraft("name", event.target.value)} /></label><label className="field-label">{label(language, "Identity", "เพศหรืออัตลักษณ์")}<input value={draft.identity} onChange={(event) => updateDraft("identity", event.target.value)} /></label></div><label className="field-label field-label--spaced">{label(language, "Origin", "ที่มา")}<input value={draft.origin} onChange={(event) => updateDraft("origin", event.target.value)} /></label><div className="field-grid field-grid--two"><label className="field-label">{label(language, "Strength", "จุดเด่น")}<input value={draft.strength} onChange={(event) => updateDraft("strength", event.target.value)} /></label><label className="field-label">{label(language, "Weakness", "จุดด้อย")}<input value={draft.weakness} onChange={(event) => updateDraft("weakness", event.target.value)} /></label></div><div className="relation-questions">{RELATIONSHIP_QUESTIONS.map(([id, question]) => <label className="field-label" key={id}><span>{question}</span><input value={draft.answers[id] ?? ""} onChange={(event) => updateAnswer(id, event.target.value)} /></label>)}</div><div className="pull-chooser"><div><SectionKicker>OPENING PULLS</SectionKicker><strong>{label(language, "Choose up to two answered ties to shape opening stats", "เลือกแรงดึงที่ตอบแล้วได้ไม่เกิน 2 ข้อ เพื่อปรับคุณลักษณะเริ่มต้น")}</strong><p>{label(language, "The occupation is the base. Chosen ties add +1 to the matching stat, up to 6.", "อาชีพคือฐาน แรงดึงที่เลือกจะเพิ่มคุณลักษณะที่เกี่ยวข้อง +1 แต่ไม่เกิน 6")}</p></div><div>{RELATIONSHIP_QUESTIONS.map(([id, question]) => { const active = activePullIds.includes(id); const unanswered = !draft.answers[id]?.trim(); return <button type="button" key={id} disabled={unanswered || (!active && activePullIds.length >= 2)} className={active ? "is-selected" : ""} onClick={() => toggleAttributePull(id)}><span>{active ? "✓" : "+"}</span><b>{question}</b><small>{unanswered ? label(language, "Answer above first", "ตอบคำถามด้านบนก่อน") : label(language, "Shapes a starting stat", "ส่งผลต่อคุณลักษณะเริ่มต้น")}</small></button>; })}</div></div><StepControls previous={() => setStep(2)} next={() => setStep(4)} /></>}
        {step === 4 && <><div className="campaign-review"><SectionKicker>LOCAL CAMPAIGN PREVIEW</SectionKicker><h2>{context.title || "Untitled Campaign"}</h2><p>{context.year} · {context.season} · {context.region}</p><hr /><strong>{draft.name || "ผู้ไร้นาม"}</strong><span>{draft.templateId === "freeform" ? draft.freeformOccupation || "ผู้เดินทางไร้สังกัด" : template.label}</span><small>{draft.origin || context.location} · {label(language, "Age", "อายุ")} {template.age}</small><div className="review-lines"><span>{label(language, "Strength", "จุดเด่น")}: {draft.strength || "—"}</span><span>{label(language, "Weakness", "จุดด้อย")}: {draft.weakness || "—"}</span></div></div><StarterTemplateDossier template={template} attributes={previewAttributes} language={language} compact /><div className="mechanic-note"><SengokuIcon name="roll" tone="ochre" /><div><strong>{label(language, "Occupation sets the base; chosen ties refine the five stats", "อาชีพกำหนดฐาน และแรงดึงที่เลือกจะขัดเกลาคุณลักษณะห้าค่า")}</strong><p>{label(language, "Players do not distribute numbers. Occupation, two chosen ties, gear, relationships, and the scene decide what can help a roll.", "ผู้เล่นไม่แจกตัวเลขเอง อาชีพ แรงดึงที่เลือกสองข้อ สัมภาระ ความสัมพันธ์ และฉาก จะเป็นตัวกำหนดสิ่งที่ช่วยการทอยได้")}</p></div></div><StepControls previous={() => setStep(3)} nextLabel={label(language, "BEGIN WITH 50 CREDITS", "เริ่มเรื่องพร้อม 50 เครดิต")} next={() => onStart(createGameState({ id: `camp-${Date.now()}`, title: context.title || "Untitled Campaign", year: context.year, season: context.season, region: context.region, location: context.location || context.region, warShadow: 3, day: 1 }, draft))} /></>}
      </section>
...
function CharacterView({ game, language, open }: { game: GameState; language: Language; open: (page: PageId) => void }) {
  ...
  return <div className="page character-view">
    ...
    <div className="character-columns"><section><div className="tab-strip">{(["traits", "masteries", "inventory", "ties"] as const).map((entry) => <button key={entry} className={tab === entry ? "active" : ""} onClick={() => setTab(entry)}>{label(language, entry === "traits" ? "Traits" : entry === "masteries" ? "Masteries" : entry === "inventory" ? "Gear" : "Ties", entry === "traits" ? "คุณลักษณะ" : entry === "masteries" ? "ความชำนาญ" : entry === "inventory" ? "สัมภาระ" : "แรงดึง")}</button>)}</div>
      {tab === "traits" && <div className="stat-grid">{STATS.map((stat) => <div className="stat-cell" key={stat.id}><SengokuIcon name="memory" tone="navy" /><strong>{language === "en" ? stat.en : stat.th}</strong><b>{game.character.attributes[stat.id]}</b><small>{localized(language, stat.hint)}</small><button>{label(language, "Used by the action parser", "ระบบเลือกใช้จากการกระทำ")}</button></div>)}</div>}
