...
function CharacterView({ game, language, open }: { game: GameState; language: Language; open: (page: PageId) => void }) {
  ...
  return <div className="page character-view">
    ...
    <div className="character-columns"><section><div className="tab-strip">{(["traits", "masteries", "inventory", "ties"] as const).map((entry) => <button key={entry} className={tab === entry ? "active" : ""} onClick={() => setTab(entry)}>{label(language, entry === "traits" ? "Traits" : entry === "masteries" ? "Masteries" : entry === "inventory" ? "Gear" : "Background", entry === "traits" ? "คุณลักษณะ" : entry === "masteries" ? "ความชำนาญ" : entry === "inventory" ? "สัมภาระ" : "ภูมิหลังตัวละคร")}</button>)}</div>
      {tab === "traits" && <><div className="stat-grid">{STATS.map((stat) => { const value = game.character.attributes[stat.id]; return <div className="stat-cell" key={stat.id}><SengokuIcon name="memory" tone="navy" /><strong>{language === "en" ? stat.en : stat.th}</strong><b>{value}</b><small>{localized(language, stat.hint)}</small><button>{label(language, `Trait value +${value} · added directly when this approach is used`, `ค่า Trait +${value} · บวกตรงเมื่อใช้แนวทางนี้`)}</button></div>; })}</div><section className="stat-xp-ledger" aria-label={label(language, "Trait rule", "กติกา Trait")}><header className="stat-xp-ledger__header"><div><SectionKicker>{label(language, "TRAIT RULE", "กติกา Trait")}</SectionKicker><h2>{label(language, "Who the character is", "ตัวละครเป็นคนแบบไหน")}</h2><p>{label(language, "Traits keep their values from 1 to 10. The engine adds the selected value directly to a roll; Traits do not have bonus tiers or their own XP track.", "Traits มีค่า 1 ถึง 10 และ engine จะบวกค่าที่เลือกตรงเข้าผลทอย ไม่มีโบนัส tier และไม่มี XP แยกของ Trait")}</p></div><span className="stat-xp-ledger__key">{label(language, "Masteries hold the only Progress track", "Mastery เป็นระบบ Progress เพียงชุดเดียว")}</span></header></section></>}
