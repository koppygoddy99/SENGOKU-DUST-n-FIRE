...
function toGMContext(game: GameState) {
  ...
  return {
    ...
    progression: {
      growthPoints: game.progression?.growthPoints ?? 0,
      milestonePoints: game.progression?.milestonePoints ?? 0,
...
import { CampaignNavigation } from "@/features/navigation/CampaignNavigation";
export { CampaignNavigation };
  ...
  return <div className={appClass}>
    ...
    <aside className={`sidebar ${menuOpen ? "sidebar--open" : ""} ${sidebarCollapsed ? "sidebar--collapsed" : ""}`}>
      ...
      <div className="sidebar__vitals">
        <Vital label={label(language, "Blood", "เลือด")} value={`${game.character.vitals.blood}/${game.character.vitals.maxBlood ?? 6}`} percent={(game.character.vitals.blood / (game.character.vitals.maxBlood ?? 6)) * 100} tone="red" />
        <Vital label={label(language, "Focus", "ค่าสติ")} value={`${game.character.vitals.focus}/${game.character.vitals.maxFocus ?? 6}`} percent={(game.character.vitals.focus / (game.character.vitals.maxFocus ?? 6)) * 100} tone="ochre" />
      </div>
...
function CharacterView({ game, language, open }: { game: GameState; language: Language; open: (page: PageId) => void }) {
  ...
  return <div className="page character-view">
    ...
    <section className="dossier-header"><div className="dossier-name"><span className="mon-avatar mon-avatar--large">火</span><div><h2>{game.character.name}</h2><p>{game.character.strength}</p></div></div><div className="dossier-resources"><span>{label(language, "Blood", "เลือด")}<strong>{game.character.vitals.blood}/{game.character.vitals.maxBlood ?? 6}</strong></span><span>{label(language, "Focus", "ค่าสติ")}<strong>{game.character.vitals.focus}/{game.character.vitals.maxFocus ?? 6}</strong></span><span>{label(language, "Trait cap", "เพดาน Trait")}<strong>10</strong></span><span>{label(language, "Rank", "ยศ")}<strong>{game.character.social.rank}</strong></span></div></section>
