  ...
  return <div className="page start-view">
    ...
    <div className="start-layout"><ol className="step-rail">{["Campaign", "Starting path", "Character", "Confirmation"].map((entry, index) => <li key={entry} className={step === index + 1 ? "is-current" : step > index + 1 ? "is-done" : ""}><span>{index + 1}</span><div><strong>{entry}</strong><small>{["time and place", "one of ten paths", "ties and strengths", "local save begins"][index]}</small></div></li>)}</ol>
      <section className="start-form">
        ...
        {step === 2 && <><div className="choice-header"><strong>{label(language, "Pick a starting path", "เลือกอาชีพเริ่มต้น")}</strong><span>{label(language, "Each path begins at age 18–20", "ทุกอาชีพเริ่มในวัย 18–20 ปี")}</span></div><div className="template-grid">{STARTER_TEMPLATES.map((entry) => <button key={entry.id} className={`template-card ${draft.templateId === entry.id ? "is-selected" : ""}`} onClick={() => { updateDraft("templateId", entry.id); updateDraft("freeformOccupation", ""); }}><span>{entry.id === draft.templateId ? "SELECTED" : "PATH"}</span><strong>{entry.label}</strong><small>{entry.short}</small><i>{entry.pressure}</i></button>)}</div><StarterTemplateDossier template={template} attributes={previewAttributes} language={language} /><label className="field-label field-label--spaced"><span>{label(language, "Or write an occupation", "หรือพิมพ์อาชีพเอง")}</span><input value={draft.freeformOccupation} placeholder={label(language, "Leave empty to use the selected path", "เว้นว่างเพื่อใช้อาชีพที่เลือก")} onChange={(event) => { updateDraft("freeformOccupation", event.target.value); if (event.target.value) updateDraft("templateId", "freeform"); }} /></label>{contextLooksUnusual && <div className="context-check"><BookOpen size={17} />{label(language, "This path is normally associated with another region. The prototype records it as a background choice rather than rewriting history.", "อาชีพนี้มักสัมพันธ์กับภูมิภาคอื่น ต้นแบบจะบันทึกเป็นภูมิหลังของตัวละครแทนการปรับประวัติศาสตร์ให้เข้ากับตัวเลือก")}</div>}<StepControls previous={() => setStep(1)} next={() => setStep(3)} /></>}
        {step === 3 && <><div className="field-grid field-grid--two"><label className="field-label">{label(language, "Character name", "ชื่อตัวละคร")}<input value={draft.name} onChange={(event) => updateDraft("name", event.target.value)} /></label><label className="field-label">{label(language, "Identity", "เพศหรืออัตลักษณ์")}<input value={draft.identity} onChange={(event) => updateDraft("identity", event.target.value)} /></label></div><label className="field-label field-label--spaced">{label(language, "Origin", "ที่มา")}<input value={draft.origin} onChange={(event) => updateDraft("origin", event.target.value)} /></label><label className="field-label field-label--spaced">{label(language, "Strength", "จุดเด่น")}<input value={draft.strength} onChange={(event) => updateDraft("strength", event.target.value)} /></label><div className="field-grid field-grid--two">{[0, 1].map((index) => <label className="field-label" key={`flaw-${index}`}><span>{label(language, `Flaw ${index + 1}${index ? " (optional)" : ""}`, `จุดอ่อน ${index + 1}${index ? " (ไม่บังคับ)" : ""}`)}</span><input value={draft.flaws?.[index] ?? ""} placeholder={index ? label(language, "A second pressure, if wanted", "แรงกดดันข้อที่สอง หากต้องการ") : label(language, "A flaw with a real cost", "จุดอ่อนที่มีราคาจริง")} onChange={(event) => { const flaws = [...(draft.flaws ?? [])]; flaws[index] = event.target.value; updateDraft("flaws", flaws); if (index === 0) updateDraft("weakness", event.target.value); }} /></label>)}</div><div className="relation-questions"><div className="field-label"><span>{label(language, "Choose 3–5 starting skills", "เลือกสกิลเริ่มต้น 3–5 อย่าง")}</span>{template.masteries.map((mastery) => <label key={mastery.id}><input type="checkbox" checked={(draft.skills ?? []).includes(mastery.label)} onChange={(event) => { const current = draft.skills ?? []; const next = event.target.checked ? Array.from(new Set([...current, mastery.label])).slice(0, 5) : current.filter((entry) => entry !== mastery.label); updateDraft("skills", next); }} /> {mastery.label}</label>)}<small>{label(language, "Selected skills begin at Step 1.", "สกิลที่เลือกเริ่มที่ Step 1")}</small></div>{RELATIONSHIP_QUESTIONS.map(([id, question]) => <label className="field-label" key={id}><span>{question}</span><input value={draft.answers[id] ?? ""} onChange={(event) => updateAnswer(id, event.target.value)} /></label>)}</div><div className="background-note"><SectionKicker>CHARACTER BACKGROUND</SectionKicker><strong>{label(language, "Flaws affect a roll only when the AI GM finds the current context directly relevant", "จุดอ่อนมีผลต่อการทอยเฉพาะเมื่อ AI GM เห็นว่าบริบทฉากเกี่ยวข้องโดยตรง")}</strong><p>{label(language, "When triggered, one flaw applies −2 before DN comparison. Otherwise it stays a remembered part of the character and no status is shown.", "เมื่อถูก trigger จุดอ่อนหนึ่งข้อจะหัก −2 ก่อนเทียบ DN นอกนั้นมันเป็นเพียงส่วนหนึ่งที่โลกจดจำและจะไม่แสดงสถานะ")}</p></div><StepControls previous={() => setStep(2)} next={() => setStep(4)} /></>}
        {step === 4 && <><div className="campaign-review"><SectionKicker>LOCAL CAMPAIGN PREVIEW</SectionKicker><h2>{context.title || "Untitled Campaign"}</h2><p>{context.year} · {context.season} · {context.region}</p><hr /><strong>{label(language, "Ready to begin", "พร้อมเริ่มเรื่อง")}</strong><small>{label(language, "Review who you are, your first mission, and what the world may remember before creating the Local Save.", "ตรวจว่าตัวละครคือใคร ภารกิจแรกคืออะไร และโลกอาจจดจำภูมิหลังใดก่อนสร้าง Local Save")}</small></div><CharacterConfirmationDossier draft={draft} template={template} attributes={previewAttributes} language={language} /><div className="mechanic-note"><SengokuIcon name="roll" tone="ochre" /><div><strong>{label(language, "Occupation sets the base; character background stays narrative", "อาชีพกำหนดฐาน ส่วนภูมิหลังตัวละครเป็นข้อมูลเชิงเรื่อง")}</strong><p>{label(language, "Players do not distribute numbers. Background may matter later only when the story naturally reaches it; it never changes Stats or roll bonuses.", "ผู้เล่นไม่แจกตัวเลขเอง ภูมิหลังอาจกลับมาเกี่ยวข้องเมื่อเรื่องพาไปถึงเท่านั้น และไม่เปลี่ยน Stat หรือโบนัสทอย")}</p></div></div><StepControls previous={() => setStep(3)} nextLabel={label(language, "BEGIN WITH 50 CREDITS", "เริ่มเรื่องพร้อม 50 เครดิต")} next={() => onStart(createGameState({ id: `camp-${Date.now()}`, title: context.title || "Untitled Campaign", year: context.year, season: context.season, region: context.region, location: context.location || context.region, warShadow: 3, day: 1 }, draft))} /></>}
      </section>
      <aside className="historical-preview"><SectionKicker>WHAT THIS PROTOTYPE DOES</SectionKicker><h2>{context.year} · {language === "en" ? context.season : seasonThai[context.season]}</h2><div className="preview-row"><SengokuIcon name="history" tone="vermilion" />{label(language, "Shows a visible context label instead of presenting fictional NPCs as historical fact.", "แสดงป้ายบริบทที่เห็นได้ แทนการนำ NPC สมมติไปอ้างเป็นข้อเท็จจริง")}</div><div className="preview-row"><SengokuIcon name="roll" tone="ochre" />{label(language, "Uses the canonical 2d12 engine, five stats, mastery, context, and momentum.", "ใช้เครื่องยนต์ 2d12 คุณลักษณะห้าค่า ความชำนาญ บริบท และแรงฮึด")}</div><div className="preview-row"><SengokuIcon name="log" tone="teal" />{label(language, "Stores the whole record locally in this browser first.", "เก็บบันทึกทั้งหมดไว้ในเบราว์เซอร์นี้ก่อน")}</div></aside>
    </div>
  </div>;
}
...
function CharacterView({ game, language, open }: { game: GameState; language: Language; open: (page: PageId) => void }) {
  ...
  return <div className="page character-view">
    ...
    <div className="character-columns"><section><div className="tab-strip">{(["traits", "masteries", "inventory", "ties"] as const).map((entry) => <button key={entry} className={tab === entry ? "active" : ""} onClick={() => setTab(entry)}>{label(language, entry === "traits" ? "Traits" : entry === "masteries" ? "Masteries" : entry === "inventory" ? "Gear" : "Background", entry === "traits" ? "คุณลักษณะ" : entry === "masteries" ? "ความชำนาญ" : entry === "inventory" ? "สัมภาระ" : "ภูมิหลังตัวละคร")}</button>)}</div>
      {tab === "traits" && <><div className="stat-grid">{STATS.map((stat) => { const value = game.character.attributes[stat.id]; return <div className="stat-cell" key={stat.id}><SengokuIcon name="memory" tone="navy" /><strong>{language === "en" ? stat.en : stat.th}</strong><b>{value}</b><small>{localized(language, stat.hint)}</small><button>{label(language, `Bonus +${bonusForStatValue(value)} · used by the action parser`, `โบนัส +${bonusForStatValue(value)} · ระบบเลือกใช้จากการกระทำ`)}</button></div>; })}</div><section className="stat-xp-ledger" aria-label={label(language, "Stat advancement", "ความก้าวหน้าของคุณลักษณะ")}><header className="stat-xp-ledger__header"><div><SectionKicker>{label(language, "STAT ADVANCEMENT", "ความก้าวหน้าของคุณลักษณะ")}</SectionKicker><h2>{label(language, "Practice becomes capability", "การฝึกกลายเป็นความสามารถ")}</h2><p>{label(language, "Only committed rolls add XP. Each Stat grows independently when its own work earns enough experience.", "เฉพาะผลทอยที่บันทึกแล้วเท่านั้นที่เพิ่ม XP แต่ละ Stat เติบโตแยกกัน เมื่อการใช้งานของมันสะสมประสบการณ์เพียงพอ")}</p></div><span className="stat-xp-ledger__key">{label(language, "XP is tracked per Stat", "XP แยกตาม Stat")}</span></header><div className="stat-xp-ledger__grid">{STATS.map((stat) => { const value = game.character.attributes[stat.id]; const progress = game.character.statXp[stat.id] ?? { xp: 0, totalXp: 0 }; const needed = statXpNeededForValue(value); const isMaxed = needed === 0; const percent = isMaxed ? 100 : Math.min(100, Math.round((progress.xp / needed) * 100)); const remaining = Math.max(0, needed - progress.xp); return <article className={`stat-xp-card ${isMaxed ? "is-maxed" : ""}`} key={stat.id}><div className="stat-xp-card__top"><span><SengokuIcon name="memory" tone={stat.id === "heart" ? "vermilion" : stat.id === "mind" ? "ochre" : "teal"} /> {language === "en" ? stat.en : stat.th}</span><strong>{label(language, "STAT", "STAT")} {value}<em>+{bonusForStatValue(value)}</em></strong></div><div className="stat-xp-card__track" role="progressbar" aria-valuemin={0} aria-valuemax={needed || 1} aria-valuenow={isMaxed ? needed : progress.xp} aria-label={label(language, `${language === "en" ? stat.en : stat.th} XP progress`, `ความคืบหน้า XP ${stat.th}`)}><span style={{ width: `${percent}%` }} /></div><div className="stat-xp-card__xp"><strong>{isMaxed ? label(language, "MAX", "เต็มขั้น") : `${progress.xp}/${needed} XP`}</strong><span>{isMaxed ? label(language, "Maximum potential reached", "ถึงศักยภาพสูงสุดแล้ว") : label(language, `${remaining} XP to reach Stat ${value + 1}`, `อีก ${remaining} XP เพื่อเพิ่ม Stat เป็น ${value + 1}`)}</span></div><footer>{label(language, `Total earned: ${progress.totalXp} XP`, `XP สะสมทั้งหมด: ${progress.totalXp}`)}</footer></article>; })}</div></section></>}
      {tab === "masteries" && <div className="mastery-list"><div className="list-title"><span>{label(language, "Mastery", "ความชำนาญ")}</span><span>{label(language, "Step & bonus", "ขั้นและโบนัส")}</span><span>{label(language, "Practice record", "บันทึกการฝึก")}</span></div>{game.character.masteries.map((entry) => { const rank = entry.rank ?? 1; const tier = masteryTierForRank(rank); return <div className="mastery-row" key={entry.id}><span><SengokuIcon name="memory" size={15} tone="ochre" />{localized(language, entry.label)}</span><strong>{label(language, "Step", "ขั้น")} {rank} · +{tier.bonus}</strong><small>{localized(language, entry.origin)}<br />{language === "en" ? tier.en : tier.th} · {rank === 20 ? label(language, "Mastered", "อาจารย์ที่เชี่ยวชาญ") : `${entry.xp ?? 0}/${xpNeededForRank(rank)} XP · DN ${tier.minimumDifficulty}+`}</small></div>; })}</div>}
...
export function campaignStepXpContext(game: GameState, language: Language) {
  ...
  if (rank >= 20) return label(language, `${leading.label}: mastered`, `${leading.label}: อาจารย์ที่เชี่ยวชาญ`);
...
function CampaignLedgerStrip({ game, language }: { game: GameState; language: Language }) {
  ...
  return <><section className="campaign-ledger-strip" aria-label={label(language, "Campaign ledger summary", "สรุปสมุดแคมเปญ")}><span><small>{label(language, "PAGE", "หน้าเรื่อง")}</small><b>{game.progression?.leaf ?? game.tick}</b></span><span><small>{label(language, "CAMPAIGN DAY", "วันในแคมเปญ")}</small><b>{game.campaign.day}</b></span><span><small>{label(language, "AGE", "อายุ")}</small><b>{game.progression?.currentAge ?? "—"}</b></span><span><small>{label(language, "HIGHEST STEP", "ขั้นสูงสุด")}</small><b>{highestMastery}/20</b></span><span><small>{label(language, "NEXT PRACTICE", "ฝึกสู่ขั้นถัดไป")}</small><b>{campaignStepXpContext(game, language)}</b></span><span><small>{label(language, "OPEN AGREEMENTS", "สัญญาค้าง")}</small><b>{openObligations}</b></span></section><p className="campaign-reward-ribbon" data-testid="campaign-reward-context"><span>{label(language, "REWARD CONTEXT", "สรุปรางวัลล่าสุด")}</span>{campaignRewardContext(game, language)}</p></>;
}
