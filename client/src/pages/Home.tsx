    ...
    <div className="page start-view">
      ...
      <div className="start-layout">
        ...
        <section className="start-form">
          {step === 1 && <><label className="field-label">ชื่อแคมเปญ<input value={draft.title} onChange={(event) => update("title", event.target.value)} placeholder="เช่น มรสุมเหนือมิกาวะ" /></label><div className="field-grid"><label className="field-label">ปี (ค.ศ.)<select value={draft.year} onChange={(event) => update("year", event.target.value)}><option>1578</option><option>1575</option><option>1580</option></select></label><label className="field-label">ฤดูกาล<select value={draft.season} onChange={(event) => update("season", event.target.value)}><option>Summer</option><option>Spring</option><option>Autumn</option></select></label><label className="field-label">ภูมิภาค<select value={draft.region} onChange={(event) => update("region", event.target.value)}><option>Mikawa</option><option>Kii</option><option>Sakai</option></select></label></div><div className="context-check"><Check size={17} /> ระบบจะใช้บริบทตัวอย่างตามปี ฤดู และพื้นที่ เพื่อทำฉากเปิดเกมในแอป UI นี้</div><div className="credit-confirm"><div><strong>ขั้นนี้ยังไม่ใช้เครดิต</strong><span>เจ้ากลับมาแก้บริบทได้ทุกเมื่อก่อนเริ่มเรื่อง</span></div><Button className="df-button df-button--primary" onClick={() => setStep(2)}>สร้างตัวละคร <ArrowRight size={18} /></Button></div></>}
