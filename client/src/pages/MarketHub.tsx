...
export function MarketHub({ game, language, onUpdate, initialTab = "market" }: { game: GameState; language: Language; onUpdate: (next: GameState, message: string) => void; initialTab?: MarketHubTab }) {
  ...
  const economy = game.economy ?? {
    ...
    sellerNetwork: copy(language, "Local sellers", "ผู้ขายในพื้นที่"),
  ...
  const xpContext = leadingMastery ? leadingRank >= 20 ? copy(language, `${leadingMastery.label}: mastered`, `${leadingMastery.label}: ถึงขีดสุด`) : `${leadingMastery.label}: ${leadingMastery.xp ?? 0}/${xpNeededForRank(leadingRank)} XP` : "—";
  ...
  const rewardContext = latestReward ? copy(language, `Latest reward context: ${latestReward}`, `รางวัลล่าสุด: ${latestReward}`) : copy(language, "No reward has entered the campaign record yet.", "ยังไม่มีรางวัลเข้าสมุดแคมเปญ");
  ...
  const localContext = <section className="market-reason market-reason--context"><MapPin size={18} /><div><strong>{economy.marketTitle}</strong><br />{economy.marketContext}<br /><small>{copy(language, "Route:", "เส้นทาง:")} {economy.routeStatus}</small></div></section>;
  ...
  return <div className="page market-view">
    ...
    <section className="campaign-ledger-strip market-concordance"><span><small>{copy(language, "LEAF", "หน้าเรื่อง")}</small><b>{game.progression?.leaf ?? game.tick}</b></span><span><small>{copy(language, "HIGHEST STEP", "ขั้นสูงสุด")}</small><b>{highestMastery}/20</b></span><span><small>{copy(language, "NEXT PRACTICE", "ฝึกสู่ขั้นถัดไป")}</small><b>{xpContext}</b></span><span><small>{copy(language, "OPEN AGREEMENTS", "สัญญาค้าง")}</small><b>{openObligations}</b></span><span><small>{copy(language, "RECORDS", "บันทึก")}</small><b>{economy.transactions.length}</b></span></section>
    <p className="campaign-reward-ribbon" data-testid="market-reward-context"><span>{copy(language, "REWARD CONTEXT", "บริบทรางวัล")}</span>{rewardContext}</p>
    {tabButtons}
    <LedgerGuidance tab={tab} language={language} marketTitle={economy.marketTitle} />

    <section data-testid="market-tab-content">
      {tab === "gear" && <><section className="inventory-sheet"><div className="inventory-capacity"><span>{copy(language, "Carried slots", "ช่องสัมภาระ")}</span><strong>{usedSlots}/8</strong></div>{game.character.inventory.map((item) => <article key={item.id}><div><strong>{item.label}</strong><small>{item.description}</small></div><span>{copy(language, item.location ?? "carried", item.location === "safehouse" ? "อยู่เซฟเฮาส์" : "พกติดตัว")}</span><b>{copy(language, item.ownership ?? "owned", item.ownership === "borrowed" ? "ยืมมา" : item.ownership === "disputed" ? "มีข้อพิพาท" : "ของตน")}</b><small className="market-row-status">{copy(language, "RECORD", "บันทึก")}</small></article>)}</section>{localContext}</>}
