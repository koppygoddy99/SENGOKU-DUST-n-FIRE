...
import { buyMarketOffer, formatMoney, inventoryCategory, xpNeededForMasteryLevel, type GameState, type InventoryCategory } from "@/lib/game";
...
export function MarketHub({ game, language, onUpdate, initialTab = "market" }: { game: GameState; language: Language; onUpdate: (next: GameState, message: string) => void; initialTab?: MarketHubTab }) {
  ...
  const inventoryCounts = useMemo(() => game.character.inventory.reduce<Record<InventoryCategory, number>>((counts, item) => { const category = inventoryCategory(item); counts[category] += 1; return counts; }, { weapon: 0, food: 0, medicine: 0, story: 0, tool: 0, status: 0 }), [game.character.inventory]);
  const visibleInventory = inventoryFilter === "all" ? game.character.inventory : game.character.inventory.filter((item) => inventoryCategory(item) === inventoryFilter);
  const inventoryFilters: { id: InventoryCategory | "all"; th: string; en: string }[] = [{ id: "all", th: "ทั้งหมด", en: "All" }, { id: "weapon", th: "อาวุธ", en: "Weapons" }, { id: "food", th: "อาหาร", en: "Food" }, { id: "medicine", th: "ยา", en: "Medicine" }, { id: "story", th: "เนื้อเรื่อง", en: "Story" }, { id: "tool", th: "เครื่องมือ", en: "Tools" }, { id: "status", th: "สถานะ", en: "Status" }];
  ...
  return <div className="page market-view">
    <div className="page-heading"><div><div className="section-kicker">MARKET & GEAR · LOCAL CONTEXT</div><h1>{copy(language, "Markets are made of people", "ตลาดคือคน ของ และคำติดค้าง")}</h1><p>{copy(language, "These five ledgers describe one changing situation: what you carry, who can help, what you owe, and which promises now shape the road ahead.", "ห้าบัญชีนี้คือเหตุการณ์เดียวกันคนละมุม: ของที่พก คนที่ช่วยได้ สิ่งที่ค้าง และคำมั่นที่กำลังเปลี่ยนทางข้างหน้า")}</p><p data-testid="prepare-campaign-context" className="market-view__campaign-context"><b>{game.campaign.title}</b> · {game.campaign.year} · {game.campaign.season} · {game.campaign.region} · {game.campaign.location}</p></div><div className="resource-tally"><span>{copy(language, "Money", "เงิน")}<b>{formatMoney(game.character.resources.currency?.amount ?? game.character.resources.property, language)}</b></span><span>{copy(language, "Food & medicine", "อาหารและยา")}<b>{inventoryCounts.food + inventoryCounts.medicine}</b></span><span>{copy(language, "Open obligations", "หนี้บุญคุณค้าง")}<b>{openObligations}</b></span></div></div>
    ...
    <section data-testid="market-tab-content">
      {tab === "gear" && <><section className="inventory-sheet"><div className="inventory-capacity"><span>{copy(language, "Carried slots", "ช่องสัมภาระ")}</span><strong>{usedSlots}/8</strong></div><div className="tab-strip" aria-label="Inventory categories">{inventoryFilters.map((filter) => <button key={filter.id} className={inventoryFilter === filter.id ? "active" : ""} onClick={() => setInventoryFilter(filter.id)}>{copy(language, filter.en, filter.th)}{filter.id !== "all" ? ` · ${inventoryCounts[filter.id]}` : ` · ${game.character.inventory.length}`}</button>)}</div>{visibleInventory.map((item) => <article key={item.id}><div><strong>{localized(language, item.label)}</strong><small>{localized(language, item.description)}</small></div><span>{copy(language, item.location ?? "carried", item.location === "safehouse" ? "อยู่เซฟเฮาส์" : "พกติดตัว")}</span><b>{copy(language, inventoryCategory(item), inventoryCategory(item) === "weapon" ? "อาวุธ" : inventoryCategory(item) === "food" ? "อาหาร" : inventoryCategory(item) === "medicine" ? "ยา" : inventoryCategory(item) === "story" ? "เนื้อเรื่อง" : inventoryCategory(item) === "tool" ? "เครื่องมือ" : "สถานะ")}</b><small className="market-row-status">{copy(language, "RECORD", "บันทึก")}</small></article>)}</section>{localContext}</>}
