...
import { buyMarketOffer, equipItem, equipmentSlotForItem, formatMoney, inventoryCategory, unequipItem, useMarketService, xpNeededForMasteryLevel, type GameState, type InventoryCategory } from "@/lib/game";
...
function LedgerGuidance({ tab, language, marketTitle }: { tab: MarketHubTab; language: Language; marketTitle: string }) {
  ...
  const message = tab === "gear"
    ...
    : tab === "market"
      ...
      : tab === "services"
        ? copy(language, "Choose a contact below to engage their help", "เลือกผู้ติดต่อด้านล่างเพื่อจ้างช่วยงาน")
        : tab === "obligations"
...
export function MarketHub({ game, language, onUpdate, initialTab = "market" }: { game: GameState; language: Language; onUpdate: (next: GameState, message: string) => void; initialTab?: MarketHubTab }) {
  ...
  const equipment = game.equipment ?? { outfit: null, weapon: null };
  const equippedName = (slot: "outfit" | "weapon") => { const id = equipment[slot]; const entry = id ? game.character.inventory.find((item) => item.id === id) : undefined; return entry ? localized(language, entry.label) : copy(language, "—", "—"); };
